	...
	public void doGet(HttpServletRequest req, HttpServletResponse resp)
		...
		try {
			...
			if (estr.getJolly()!=null && estr.getJolly().length()>0) {
				resp.getWriter().print("L'estrazione è valida (Jolly=["+estr.getJolly()+"]<br>");
				...
				if (!estr.getDataestrazione().equals(dataUltimaEstrazione)) {
					...
					resp.getWriter().println("Non è necessario aggiornare la banca dati.<br>");
				}
				
			} else {
				resp.getWriter().print("Lo scarico dati non è stato completato.<br>");
			}
		} catch (Exception e) {
