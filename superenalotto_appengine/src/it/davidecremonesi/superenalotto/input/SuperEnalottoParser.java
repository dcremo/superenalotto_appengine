...
/**
 * Il primo parser, ora abbandonato
 * Leggeva i dati dalla struttura HTML della pagina del sito 
 * giochinumerici.sisal.it
 * Sebbene molto incasinata, ha funzionato per oltre cinque anni.
 * 
 * @author admin
 * @deprecated
 */
	...
	public Estrazione getEstrazione(int anno,int n) throws Exception {
		...
		try {
			...
			if (found >-1) {
				...
				String giorno="";
				if (sub.indexOf("Lune")!=-1) giorno = "Luned�";
				if (sub.indexOf("Mart")!=-1) giorno = "Marted�";
				if (sub.indexOf("Merc")!=-1) giorno = "Mercoled�";
				if (sub.indexOf("Giov")!=-1) giorno = "Gioved�";
				if (sub.indexOf("Vene")!=-1) giorno = "Venerd�";
...
<!--
<div class="UltimoConcorsoInfo_boxShowHideText" style="margin-bottom:10px;">CONCORSO n.66 di Venerd� 03/06/2011</div>
-->

<div id="infoVinc_boxRoot" class="infoVinc_boxRoot">
    <div id="infoVinc_boxTitolo" class="infoVinc_boxTitolo">CONCORSO n.66 di Venerd� 03/06/2011</div>
...
dei premi di tutte le categorie sono state calcolate con la modalit� a totalizzatore secondo le
